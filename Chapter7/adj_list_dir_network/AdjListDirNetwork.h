...
// 抽象数据类型方法声明及重载编译系统默认方法声明:
	...
	void DFS(int v) const;                             // 深度优先搜索有向网
	void BFS(int v) const;                             // 广度优先搜索有向网
	bool Isconnected_dfs(int v1, int v2) const;  // 判断v1,v2否连通，采用深度优先搜索算法
	bool Isconnected_bfs(int v1, int v2) const;  // 判断v1,v2否连通，采用广度优先搜索算法
	bool IspathA(int v,int A) const;                   // 判断是否存在一条从v开始长度为A的简单路径
	bool Ispath(int A) const;                          // 判断是否存在一条长度为A的简单路径
...
//	cout << vexTable[v].data << " ";
	for(AdjListNetworkArc<WeightType> *p = vexTable[v].firstarc; p != NULL; p = p->nextarc)
	{
...
template<class ElemType, class WeightType>
void AdjListDirNetwork<ElemType, WeightType>::BFS(int v) const
// 操作结果：从顶点v出发进行广度优先遍历
{
	tag[v] = VISITED;
//	cout << vexTable[v].data << " ";
	...
	{
		Q.DelQueue(u);
		for(AdjListNetworkArc<WeightType> *p = vexTable[u].firstarc; p != NULL; p = p->nextarc)
		{
			if(tag[p->adjVex] == UNVISITED)
			{
...
template <class ElemType, class WeightType>
bool AdjListDirNetwork<ElemType, WeightType>::Isconnected_dfs(int v1, int v2) const
{
	...
	this->DFS(v1);
	return tag[v2] == VISITED;
}

template <class ElemType, class WeightType>
bool AdjListDirNetwork<ElemType, WeightType>::Isconnected_bfs(int v1, int v2) const
{
	for(int i = 0; i < vexNum; i++)
	{
		tag[i] = UNVISITED;
...
template <class ElemType,class WeightType>
bool AdjListDirNetwork<ElemType, WeightType>::IspathA(int v,int A) const
{
	if(A < 0 || A > vexNum)
	{
		return false;
	...
	for(AdjListNetworkArc<WeightType> *p = vexTable[v].firstarc; p != NULL; p = p->nextarc)
	{
		if(tag[p->adjVex]==UNVISITED && IspathA(p->adjVex,A-1))
		{
			return true;
		}
	}
...
template <class ElemType,class WeightType>
bool AdjListDirNetwork<ElemType, WeightType>::Ispath(int A) const
{
	for(int i = 0; i < vexNum; i++)
	{
