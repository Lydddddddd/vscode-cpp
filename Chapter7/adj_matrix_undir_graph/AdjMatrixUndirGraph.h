...
// 邻接矩阵类型的方法声明:
	...
	int CountArc(int k,int v1,int v2) const;		 // 求顶点v1和v2之间长度为k的条数	 
	int countPaths(int k,int i,int j) const;
	void DFS_count(int k,int i,int j,int &count) const;
	...
	AdjMatrixUndirGraph<ElemType> &operator =(const AdjMatrixUndirGraph<ElemType> &g);  // 赋值语句重载
...
template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::CountArc(int k,int v1,int v2) const
{
	int r[k+1][vexNum][vexNum];
...
template<class ElemType>
void AdjMatrixUndirGraph<ElemType>::DFS_count(int k,int i,int j,int &count) const
{
	if (k == 0  ) {
		if(i == j)
        count++;
	...
	for (int u = 0; u < vexNum; u++)
	{
		...
		if (arcs[i][u] && tag[u]==UNVISITED) {
            DFS_count(k-1,u, j, count);
        }
	}
    tag[i] = UNVISITED;
}
...
template<class ElemType>
int AdjMatrixUndirGraph<ElemType>::countPaths(int k, int i, int j) const
{
    int count = 0;
	DFS_count(k, i, j, count);
    return count;
}
