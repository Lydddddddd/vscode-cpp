...
//	辅助函数:
	...
	void InOrder(BinTreeNode<ElemType>*r,void (*Visit)(const ElemType &)) const;
		// 中序遍历以r为根的二叉树
	void InOrder1(BinTreeNode<ElemType>*r,void (*Visit)(const ElemType &)) const;
		// 非递归中序遍历以r为根的二叉树
...
//  二叉树方法声明及重载编译系统默认方法声明:
	...
	void InOrder1(void (*Visit)(const ElemType &)) const;// 二叉树的非递归中序遍历
	...
	int Depth() const;			// 求二叉树的最大宽度
	int LeafCount(BinTreeNode<ElemType> *p) const;		// 求二叉树的叶子结点个数
};
...
template <class ElemType>
int BinaryTree<ElemType>::LeafCount(BinTreeNode<ElemType> *p) const
{
	if(p==NULL)
	{
		return 0;
	...
	return LeafCount(p->leftChild)+LeafCount(p->rightChild);
}
...
template <class ElemType>
void BinaryTree<ElemType>::InOrder1(BinTreeNode<ElemType> *r, void (*Visit)(const ElemType &)) const
{
...
template <class ElemType>
void BinaryTree<ElemType>::InOrder1(void (*Visit)(const ElemType &)) const
// 操作结果：中序遍历二叉树
{
	InOrder1(root, Visit);	
}	
