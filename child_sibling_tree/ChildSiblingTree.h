...
//  孩子兄弟表示树类方法的声明:
	...
	int LeafCount(ChildSiblingTreeNode<ElemType> *cur) const;		// 求树的叶子个数
...
template <class ElemType>
int ChildSiblingTree<ElemType>::LeafCount(ChildSiblingTreeNode<ElemType> *cur) const
{
	ChildSiblingTreeNode<ElemType> *p;
	if (cur == NULL)
	{
	...
	else	
	{
		int count = 0;
		for (p = FirstChild(cur); p != NULL;  p = NextSibling(p))
			count += LeafCount(p);			// 对各子树的叶结点个数进行累加
		return count;
	}
}
