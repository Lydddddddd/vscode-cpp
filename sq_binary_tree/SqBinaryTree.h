...
//  顺序二叉树的函数成员:
	...
	SqBinaryTree<ElemType> &operator=(const SqBinaryTree<ElemType>& t);		
		// 赋值运算符重载
	int CommonAncestor(int r,ElemType p,ElemType q) const; // 求p，q的最低公共祖先
	void CommonAncestor(int i,int j) const;
};
...
// 顺序二叉树类的实现部分
template <class ElemType>
int SqBinaryTree<ElemType>::CommonAncestor(int r,ElemType p,ElemType q) const
{
	int left,right;
	if(elems[r]==p||elems[r]==q)
	{
		return r;
	}
	if(tag[r]==0)
	...
	left=CommonAncestor(LeftChild(r),p,q);
	right=CommonAncestor(RightChild(r),p,q);
	if(left==-1&&right==-1)
	{
		return -1;
	}
...
template <class ElemType>
void SqBinaryTree<ElemType>::CommonAncestor(int i,int j) const
{
	if(tag[i]==0)
	{
		cout<<"A[i]为空！";
